...
public class AesCipherUtil {
	...
	// 16 位字符串
	private static final String IV_PARAM_AES = "EIJ78&*^IK78#!39";
	...
	public static void main(String[] args) {
		...
		key = generateAESKey(128, "AEDIEWWOD(#$)**#$JLSJDFffsdfafsdfasdfaddd");
		System.out.println("生成的随机密钥为：" + key);
		long startTime = System.currentTimeMillis();
		System.out.println("开始时间：" + startTime);
		for (int i = 0; i < 500000; i ++) {
			password = aesEncrypt(data + i, key);
		}
		...
		String ss = password;
		for (int i = 0; i < 500000; i ++) {
			password = aesDecrypt(ss, key);
		}
		System.out.println("解密时间：" + (System.currentTimeMillis() - startTime1));
		System.out.println("解密时间：" + password);
